...
public class ProgramChooserController {
    ...
    private void displayProgram() throws IOException {
        ...
        if(selectedStatement==null)
        {
            ...
            try{
                ...
                ProgramState state=new ProgramState(new MyStack<>(), new MyDictionary<>(), new MyList<>(), new MyDictionary<>(), new MyHeap(), selectedStatement);
                IRepo repo=new Repo(state,"log"+(id+1)+".txt");
                Controller controller= new Controller(repo);
                programExecutorController.setController(controller);
            }catch(IOException | StatementExecutionException | ExpressionEvaluationException |
                   ADTException exception)
            {
                Alert alert=new Alert(Alert.AlertType.ERROR);
                alert.setTitle("ERROR");
    ...
    private ObservableList<IStatement> getAllStatements() {
        ...
        IStatement ex1 = new CompoundStatement(new VariableDeclarationStatement("v", new IntType()),
                new CompoundStatement(new AssignStatement("v", new ValueExpression(new StringValue("2"))),
